...
objDesc["Bonus block near projectiles: "] = "附近有抛射物时获得额外格挡："
...
objDesc["#YELLOW#On shield block:#LAST#"] = "#YELLOW#格挡时：#LAST#"
...
objDesc["All enemies in radius 2 take 20 fire damage each turn and healing you for 10% of the damage dealt."] = "附近2码范围的敌人每回合受到20火焰伤害。你受到10%伤害值的治疗"
	...
	for i = 1,#desc do
		if type(desc[i]) == "string" then
			...
			desc[i] = desc[i]:gsub("chance of physical repulsion","几率物理击退"):gsub("Unnatural","非自然生物"):gsub("Undead","不死族"):gsub("Demon","恶魔"):gsub("Major","大型"):gsub("Minor","小型")
					...
					:gsub("Horror","恐魔"):gsub("Dragon","龙"):gsub("Canine","犬类"):gsub("Unliving","不死生物"):gsub("Living","活物"):gsub("Giant","巨人")
