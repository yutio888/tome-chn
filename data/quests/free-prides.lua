questCHN["Children of Garkul, Unite!"] = {
	...
	description = function(desc)
		...
		desc = desc:gsub("The internment camp is located somewhere to the north. The orcs are subdued into obedience by a powerful #{halfling}# psionic, Mindwall, and guarded by elite Sunwall troops.","他们的拘留营在北部的某处。兽人们的心灵被一个强大的半身人超能力者，意志之墙所控制，并被太阳堡垒的精英部队重重把守。")
		desc = desc:gsub("Mindwall must be taken care of and the prides set free.","必须干掉意志之墙，我们的部落才能迎来自由")
		desc = desc:gsub("But #{bold}#BEFORE#{normal}# that you should go and destroy the Sunwall Observatory to the east, as our spies have found a way to resist Mindwall's psionic powers which requries #{italic}#ingredients#{normal}# from there.","但是#{bold}#之前#{normal}#你最好去一趟东部的太阳堡垒观星台。我们的间谍收到线报称，那里获得到的#{italic}#材料#{normal}#有助于找出抵御意志之墙的精神能力的方法。")
		desc = desc:gsub("You have destroyed Mindwall body but he managed to split his mind into many pieces and taken direct control of the subdued orcs. Destroy the pillars in each level four other levels.","你摧毁了意志之墙的身躯，但他找到了一种办法把自己的灵魂分成许多块，分别控制那些被关押的兽人。摧毁其他四个关卡的水晶柱。")
