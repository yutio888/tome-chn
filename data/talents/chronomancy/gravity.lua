...
Talents.talents_def.T_REPULSION_BLAST.name = "排斥冲击"
Talents.talents_def.T_REPULSION_BLAST.info = function(self, t)
		...
		format(radius, t.getDamage(self, t))
	end
	
Talents.talents_def.T_GRAVITY_SPIKE.name = "重力钉刺"
Talents.talents_def.T_GRAVITY_SPIKE.info = function(self, t)
		local damage = t.getDamage(self, t)
		local radius = self:getTalentRadius(t)
		...
		:format(radius, damage, damage/8,  damage/2)
	end

Talents.talents_def.T_GRAVITY_LOCUS.name = "重力核心"	
Talents.talents_def.T_GRAVITY_LOCUS.info = function(self, t)
		...
		return ([[在 你 身 边 制 造 一 个 重 力 场 ，将 你 造 成 伤 害 的  %d%%  转 化 为 物 理 伤 害 ，  使 向 你 发 射 的 飞 行 物 减 速  %d%%，并 使 你 免 受 重 力 伤 害 和 效 果 的 影 响 。
		 此 外 ，排 斥 冲 击 对 目 标 造 成 伤 害 之 后 ，有  %d%%  的 几 率 将 目 标 的 击 退 抗 性 减 半 两 回 合 。]]):format(conv, proj, anti)
	end

Talents.talents_def.T_GRAVITY_WELL.name = "重力之井"
Talents.talents_def.T_GRAVITY_WELL.info = function(self, t)
		...
		受 法 术 强 度 影 响 ， 伤 害 按 比 例  加 成 。]]):format(radius, duration, damage, slow*100)
	end
