...
newTalent{
	...
	info = function(self, t)
		return ([[召 唤 伟 大 的 恶 魔 领 主 乌 鲁 洛 克 的 实 体， 转 化 为 恶 魔。 
		 ...
		 当 你 处 于 恶 魔 形 态 时， 死 亡 空 间 的 火 焰 会 治 疗 你。 
...
newTalent{
	...
	info = function(self, t)
		return ([[召 唤 一 部 分 恶 魔 空 间 与 现 有 空 间 交 叉。 
		 你 的 目 标 和 你 自 己 都 会 被 带 入 恶 魔 空 间， 只 有 当 你 中 断 技 能 或 目 标 死 亡 时， 限 制 解 除。 
		 在 恶 魔 空 间 内， 永 恒 之 焰 会 燃 烧 你（ 治 疗 恶 魔） 和 目 标， 造 成 %0.2f 火 焰 伤 害。 
		 当 技 能 中 断 时， 只 有 你 和 目 标（ 如 果 还 活 着） 会 被 带 回 原 来 空 间， 所 有 的 召 唤 物 会 停 留 在 恶 魔 空 间。 
		 当 你 已 处 于 恶 魔 空 间 时， 此 技 能 施 放 无 效 果。 
