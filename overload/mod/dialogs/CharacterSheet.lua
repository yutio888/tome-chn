...
_M.immune_labels = {poison = "毒素",
	disease = "疾病", cut = "流血", confusion= "混乱",
	blind = "致盲", silence = "沉默", disarm = "缴械",
	pin = "定身", stun = "震慑/冻结", sleep = "睡眠",
	fear = "恐惧", knockback = "击退", stone = "石化",
	instakill = "即死", teleport = "传送",
	negative_status_effect 			= "#GOLD#全体     ",
	mental_negative_status_effect 	= "#ORANGE#精神负面效果  ",
	physical_negative_status_effect = "#ORANGE#物理负面效果 ",
	spell_negative_status_effect 	= "#ORANGE#魔法负面效果 ",
}
...
降低对手对你造成的伤害的抗性。]]
	...
	elseif kind=="defence" then
		...
		self:mouseTooltip(self.TOOLTIP_CRIT_REDUCTION,s:drawColorStringBlended(self.font,           ("暴击减免率      ： #00ff00#%s"):format(text), w, h, 255, 255, 255, true)) h = h + self.font_h
		text = compare_fields(player, actor_to_compare, function(actor, ...) return actor:attr("ignore_direct_crits") or 0 end, "%d%%", "%+.0f%%")
		self:mouseTooltip(self.TOOLTIP_CRIT_SHRUG,s:drawColorStringBlended(self.font,           ("暴击摆脱    ： #00ff00#%s"):format(text), w, h, 255, 255, 255, true)) h = h + self.font_h
		...
		local function sort_talents()
			...
			if self.talent_sorting == 1 then -- Grouped by talent type, Racial/infusions first, then alphabetical order
				...
				get_group = function(t, tt)
					...
					return (t_talent_cat[cat] or cat) .."/"..(tt.name or ""):bookCapitalize().." ("..mastery..")"
				end
