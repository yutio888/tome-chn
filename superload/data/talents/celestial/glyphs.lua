...
registerTalentTranslation{
	...
	info = function(self, t)
		...
		return ([[当你法术暴击时，你消耗 5 点正能量和负能量，在范围 %d 内的随机目标周围 1 码半径的区域内放置随机圣印。
		...
		可用的圣印如下所示：
		阳光圣印——将阳光注入圣印。当其触发时，将会释放出明亮的光辉，造成 %0.2f 光系伤害，并恢复你 %d 生命值。
		月光圣印——将月光注入圣印。当其触发时，将会释放出让人疲劳的黑暗，造成 %0.2f 暗影伤害，并使目标所造成的伤害减少 %d%% ，持续 %d 回合。
		暮光圣印——将暮光注入圣印。当其出发时，将会释放出一团暮光，造成 %0.2f 光系和 %0.2f 暗影伤害，并将目标击退 %d 格。
...
registerTalentTranslation{
	...
	name = "神圣之印",
	info = function(self, t)
		return ([[当你的圣印触发时，你将会获得一层星空能量的潮涌，增加你的暗影、光系抵抗和伤害吸收各 5%% ，持续 %d 回合，最多叠加 %d 次。该效果每回合最多触发 3 次。]]):format(t.getTurns(self, t), t.getMaxStacks(self, t))
	end,
}
registerTalentTranslation{
	id = "T_TWILIGHT_GLYPH",
	...
	info = function(self, t)
		...
		return ([[将暮光能量注入圣印，使其消逝并立刻造成 %d 光系或 %d 暗影伤害。
		你可以连续使用 %d 次该技能而不会使技能进入冷却，但每次使用都会增加其冷却时间2回合。当触发次数满，或你在1回合内没有使用这个技能，这个技能将会立刻进入冷却。]]):format(damDesc(self, DamageType.LIGHT, dam), damDesc(self, DamageType.DARKNESS, dam), t.getConsecutiveTurns(self, t))
	end,
}
return _M
