...
registerTalentTranslation{
	...
	info = function(self, t)
		...
		凶残猎手触发时会消耗 #ffa0ff#8 点仇恨值 #LAST# ，不会当你位于诅咒瘴气中时触发。]]):format(t.getMiasmaCount(self, t), self:getTalentRadius(t), self:damDesc(DamageType.DARKNESS, t.getDamage(self, t)/2) + self:damDesc(DamageType.MIND, t.getDamage(self, t)/2), t.getChance(self, t))
	end,
}
...
registerTalentTranslation{
	...
	name = "障壁猎手",
	info = function(self, t)
		return ([[当你被包裹在诅咒瘴气中时，你获得潜行 (%d 强度) ，并获得 %d 物理强度。
		潜行强度和物理强度受精神强度加成。]]):format(t.getStealthPower(self, t), t.getPhysPower(self, t))
	end,
}
