...
同时额外除去对应类型的越层负面效果（失去平衡、法术冲击和锁脑）。		]]):format(change_infusion_eff(what), data.power+data.inc_stat, data.dur)
	end,
	short_info = function(self, t)
		local data = self:getInscriptionData(t.short_name)
...
registerInscriptionTranslation{
	...
	info = function(self, t)
		return ([[激活符文，立刻清除你身上的负面效果。
			...
			如果只清除了越层负面效果，不会产生护盾且冷却时间减少75%%]]):format(t.getShield(self, t) * (100 + (self:attr("shield_factor") or 0)) / 100)
	end,
	short_info = function(self, t)
		local data = self:getInscriptionData(t.short_name)
