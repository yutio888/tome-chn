...
function _M:findInAllInventories(name, getname)
	...
	if (not (a and b)) and require("data-chn123.objects.artifact").artifactCHN[name] then
		a, b, c = _M.old_findInAllInventories(self, require("data-chn123.objects.artifact").artifactCHN[name].name, getname) or _M.findInAllInventoriesBy(self, "originName", name)
	end
	return a, b, c
end
return _M